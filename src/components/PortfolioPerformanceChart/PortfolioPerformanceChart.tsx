  ...
  return (
    <section className={styles.section} aria-labelledby="performance-title">
      <div className={styles.header}>
        ...
        {latest && (
          <strong className={latest.returnRate >= 0 ? styles.positive : styles.negative}>
            {latest.returnRate >= 0 ? '+' : ''}
            {latest.returnRate.toFixed(2)}%
          </strong>
        )}
      </div>
      ...
      {!isLoading && !data.length && (
        <p className={styles.status}>현재가를 새로고침하면 최근 일봉 기준 추이를 표시합니다.</p>
      )}
        ...
        <>
          <div className={styles.metrics}>
            <span>
              시작 평가금액{' '}
              <strong>
                {data[0].value.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원
              </strong>
            </span>
            <span>
              현재 평가금액{' '}
              <strong>
                {latest!.value.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원
              </strong>
            </span>
            <span>
              기간 중 최저 수익률 <strong className={styles.negative}>{lowest.toFixed(2)}%</strong>
            </span>
          </div>
