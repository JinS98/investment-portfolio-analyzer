...
export async function searchStocks(query: string, signal: AbortSignal): Promise<StockSearchItem[]> {
  ...
  if (!response.headers.get('content-type')?.includes('application/json')) throw new Error('검색 서버에 연결할 수 없습니다.');
  const data = object(await response.json());
  if (!response.ok) throw new Error(typeof data.message === 'string' ? data.message : '종목 검색에 실패했습니다.');
  if (!Array.isArray(data.result)) throw new Error('잘못된 검색 결과입니다.');
  return data.result.map((value: unknown) => {
    const row = object(value);
    if (typeof row.symbol !== 'string' || typeof row.name !== 'string' || typeof row.market !== 'string') throw new Error('잘못된 종목 정보입니다.');
...
async function request(endpoint: string, params: URLSearchParams): Promise<unknown> {
  const response = await fetch('/api/toss/' + endpoint + '?' + params, { signal: AbortSignal.timeout(40_000) });
  if (!response.headers.get('content-type')?.includes('application/json')) throw new Error('API ??? ????. npm run dev? ??????.');
  const data = object(await response.json());
...
function symbols(tickers: string[]): URLSearchParams {
  ...
  if (values.length > 200 || values.some((value) => !/^[A-Z0-9.-]+$/.test(value))) throw new Error('??? ?? ??? ?? 200? ??????.');
...
export async function fetchCandlePage(ticker: string, count = 90, before?: string) {
  symbols([ticker]);
  if (!Number.isInteger(count) || count < 1 || count > 200) throw new Error('?? ??? 1~200?? ??????.');
...
export async function fetchClosePriceAt(ticker: string, targetDate: string): Promise<number | null> {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate) || !Number.isFinite(Date.parse(targetDate))) throw new Error('??? YYYY-MM-DD? ??????.');
