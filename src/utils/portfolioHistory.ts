...
export const calcPortfolioHistory = (
  ...
  const positions = Object.values(
    portfolio.reduce<Record<string, Position>>((result, stock) => {
  ...
  const dates = [
    ...new Set(
      positions.flatMap((position) => historicalData[position.ticker].map((candle) => candle.date)),
    ),
  ].sort();
  const lastPrices: Record<string, number | null> = Object.fromEntries(
    positions.map((position) => [position.ticker, null]),
  );
  const multiplier = (market: StockItem['market']) =>
    market === 'US' ? (exchangeRate?.rate ?? 0) : 1;
  ...
  for (const date of dates) {
    ...
    const value = positions.reduce(
      (sum, position) =>
        sum + lastPrices[position.ticker]! * position.quantity * multiplier(position.market),
      0,
    );
