...
export const createRiskGuide = (riskData: RiskData): RiskGuide => {
  ...
  const concentrationScore =
    Math.min((riskData.maxWeight / 70) * 20, 20) +
    Math.min((riskData.concentration / 100) * 10, 10);
  ...
  return {
    ...
    summary:
      level === 'high'
        ? '가격 변동과 종목 집중도가 높은 구성입니다.'
        : level === 'medium'
          ? '일부 위험 지표를 주기적으로 확인해 주세요.'
          : '현재 기준으로 주요 위험 지표가 비교적 낮습니다.',
    alerts,
  };
};
